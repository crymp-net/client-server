...
void CNetLerper::AddNewPoint(const Vec3& inPos, const Vec3& inVel, const Vec3& entityPos, EntityId standingOn)
{
	...
	{
		if ((m_pPlayer != NULL) && (m_pPlayer->GetHealth() > 0))
		{
...
void CNetLerper::DebugDraw(const SPrediction& prediction, const Vec3& entityPos, bool hadNewData)
{
	...
	{
		CryLogAlways("Cur: (%.2f, %.2f, %.2f) Des: (%.2f, %.2f, %.2f) Pred: (%.2f, %.2f, %.2f) ", entityPos.x, entityPos.y, entityPos.z, desiredPosition.x, desiredPosition.y, desiredPosition.z, predictedPosition.x, predictedPosition.y, predictedPosition.z);
		CryLogAlways("LerpErrorXY: (%.2f)", m_lerpedError.GetLength2D());
		CryLogAlways("InputSpeed: (%.2f, %.2f, %.2f) lerpVel: (%.2f, %.2f, %.2f) lerpSpeed: %.2f", desiredVelocity.x, desiredVelocity.y, desiredVelocity.z, lerpVel.x, lerpVel.y, lerpVel.z, lerpVel.GetLength());
	}	
	if (hadNewData)
	{
...
CNetLerper::SSettings CNetEnhancedPlayerInput::m_lerperSettings;


CNetEnhancedPlayerInput::CNetEnhancedPlayerInput(CPlayer * pPlayer )
: 
	m_pPlayer(pPlayer),
...
void CNetEnhancedPlayerInput::UpdateInterpolation()
{
	...
	// Should Snap
	if (prediction.shouldSnap && prediction.predictedPos.IsZero())
	{
...
void CNetEnhancedPlayerInput::UpdateMoveRequest()
{
	//Code below doesnt work for X rotation
	...
	float fRotZ			= acos(clamp(fRotZDot, -1.0f, 1.0f));

	fRotZ = AngleWrap_PI(fRotZ);

	float fRotZFinal	= -sign(fRotZDirDot) * fRotZ * fNetAimLerpFactor;

	float fCurrentAngle = acos(clamp(
		Vec3Constants<float>::fVec3_OneZ.dot(m_lookDir), -1.0f, 1.0f));
	float fNewAngle		= acos(clamp(
...
*/
	...
	//Using default crymp code for now
...
void CNetEnhancedPlayerInput::PreUpdate()
{
	IPhysicalEntity * pPhysEnt = m_pPlayer->GetEntity()->GetPhysics();

	if (pPhysEnt && m_pPlayer->GetHealth() > 0 && m_pPlayer->GetLinkedVehicle()==NULL)
	{
		if (/*m_pPlayer->AllowPhysicsUpdate(m_curInput.physCounter) && */!m_pPlayer->IsClient() && HasReceivedUpdate())
		{
			UpdateInterpolation();
		}
		else
...
void CNetEnhancedPlayerInput::Update()
{
...
void CNetEnhancedPlayerInput::PostUpdate()
{
}

void CNetEnhancedPlayerInput::SetState( const SSerializedPlayerInput& input )
{
	DoSetState(input);
...
void CNetEnhancedPlayerInput::GetState( SSerializedPlayerInput& input )
{
	input = m_curInput;
}

void CNetEnhancedPlayerInput::Reset()
{
...
void CNetEnhancedPlayerInput::DisableXI(bool disabled)
{
}

float CNetEnhancedPlayerInput::CalculatePseudoSpeed() const
{
...
void CNetEnhancedPlayerInput::DoSetState(const SSerializedPlayerInput& input )
{
	...
	if (g_pGameCVars->g_debugNetPlayerInput & 1)
	{
		...
		pPD->Begin( (string("net_player_input_") + (string)m_pPlayer->GetEntity()->GetName()).c_str(), true );
...
void CNetEnhancedPlayerInput::UpdateDesiredVelocity()
{
