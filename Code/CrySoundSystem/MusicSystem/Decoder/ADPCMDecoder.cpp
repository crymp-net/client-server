...
//////////////////////////////////////////////////////////////////////////
CADPCMDecoder::CADPCMDecoder(IMusicSystem *pMusicSystem, CMusicPattern *pMusicPattern) : m_bOpen(false)
{
...
inline FILE* CADPCMDecoder::GetNewFileHandle()
{
	...
	FILE *pNewFile = m_pPak->FOpen(m_szFilename.c_str(), "rbx");
	if (!pNewFile)
		return NULL;
	return pNewFile;
}
...
//////////////////////////////////////////////////////////////////////////
inline bool CADPCMDecoder::Open(const char *sFileName, const char *sSoundName)
{
	m_FileInfo.sFileName			= sFileName;
	m_FileInfo.sSoundName			= sSoundName;
	m_szFilename							= sFileName;//damn it, but how can i get a const char* from the other string type, why do we have an own?
...
//////////////////////////////////////////////////////////////////////////
inline bool CADPCMDecoderInstance::Seek0(const int nDelayInSamples)
{
	FRAME_PROFILER( "MusicSystem::ADPCM-Decoder", GetISystem(), PROFILE_MUSIC );	

	m_nPos = -nDelayInSamples;	//new sample pos
...
inline bool CADPCMDecoderInstance::SeekPos(const int nSamplePos)
{
...
{
	//read block
    unsigned short bytesRead( (unsigned short) m_pDecoder->m_pPak->FRead( m_aEncodedBlock, m_uiCurrentBlockSize, m_pFile ) );
...
void CADPCMDecoderInstance::GetMemoryUsage(class ICrySizer* pSizer)
{
	if (!pSizer->Add(*this))
		return;
}
