...
#include "PCMDecoder_info.h"

CPCMDecoder::CPCMDecoder(IMusicSystem *pMusicSystem, CMusicPattern *pMusicPattern) : m_b44KHz(true)
{
...
bool CPCMDecoder::Open(const char *sFileName, const char *sSoundName)
{
	if (m_pFile)
		return false;

	// x for opening with direct streaming.
	m_pFile = m_pPak->FOpen(sFileName, "rbx");
...
bool CPCMDecoderInstance::Seek0(const int nDelayInSamples)
{
	FRAME_PROFILER( "MusicSystem::PCM-Decoder", GetISystem(), PROFILE_MUSIC );	
...
void CPCMDecoderInstance::GetMemoryUsage(class ICrySizer* pSizer)
{
	if (!pSizer->Add(*this))
		return;
}
