...
HRESULT Buffer::setLocation(LocStatus locStatus) noexcept
{
    ...
    else
    {
        ...
        {
            static std::array<GUID,_EAX40_MAX_ACTIVE_FXSLOTS> NullSlots{};

            const auto slots = std::as_writable_bytes(std::span{NullSlots});
            EAXSetDirect(mContext, &_EAXPROPERTYID_EAX40_Source, EAXSOURCE_ACTIVEFXSLOTID, mSource,
    ...
    ULONG *pcbReturned) noexcept
{
    ...
    *pcbReturned = 0;
    ...
    auto self = impl_from_base();
    std::lock_guard lock{self->mMutex};
    if(guidPropSet == _EAXPROPERTYID_EAX40_Source
        || guidPropSet == _DSPROPSETID_EAX30_BufferProperties
        || guidPropSet == _DSPROPSETID_EAX20_BufferProperties
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot0
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot1
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot2
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot3
        || guidPropSet == _DSPROPSETID_EAX30_ListenerProperties
        || guidPropSet == _DSPROPSETID_EAX20_ListenerProperties
        || guidPropSet == _EAXPROPERTYID_EAX40_Context
        || guidPropSet == _DSPROPSETID_EAX10_ListenerProperties
        || guidPropSet == _DSPROPSETID_EAX10_BufferProperties)
    {
    ...
    void *pInstanceData, ULONG cbInstanceData, void *pPropData, ULONG cbPropData) noexcept
{
    ...
    else if(guidPropSet == _EAXPROPERTYID_EAX40_FXSlot0
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot1
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot2
        || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot3
        || guidPropSet == _DSPROPSETID_EAX30_ListenerProperties
    ...
    *pTypeSupport = 0;
    ...
    if(self->mParent.haveExtension(EXT_EAX))
    {
        auto res = std::invoke([&guidPropSet,dwPropID]() -> std::optional<DWORD>
        {
            ...
            if(guidPropSet == _DSPROPSETID_EAX30_BufferProperties)
                return EAX3Buffer_Query(dwPropID);
            if(guidPropSet == _DSPROPSETID_EAX20_BufferProperties)
                return EAX2Buffer_Query(dwPropID);
            if(guidPropSet == _EAXPROPERTYID_EAX40_FXSlot0
                || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot1
                || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot2
                || guidPropSet == _EAXPROPERTYID_EAX40_FXSlot3)
                return EAX4Slot_Query(dwPropID);
            if(guidPropSet == _DSPROPSETID_EAX30_ListenerProperties)
                return EAX3_Query(dwPropID);
            if(guidPropSet == _DSPROPSETID_EAX20_ListenerProperties)
                return EAX2_Query(dwPropID);
            if(guidPropSet == _EAXPROPERTYID_EAX40_Context)
                return EAX4Context_Query(dwPropID);
            if(guidPropSet == _DSPROPSETID_EAX10_ListenerProperties)
                return EAX1_Query(dwPropID);
            if(guidPropSet == _DSPROPSETID_EAX10_BufferProperties)
