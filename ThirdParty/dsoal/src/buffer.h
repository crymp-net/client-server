 ...
 */
struct _BufferSubList {
    uint64_t mFreeMask{~0_u64};
    gsl::owner<std::array<Buffer,64>*> mBuffers{nullptr};

    _BufferSubList() noexcept = default;
    _BufferSubList(const _BufferSubList&) = delete;
    _BufferSubList(_BufferSubList&& rhs) noexcept : mFreeMask{rhs.mFreeMask}, mBuffers{rhs.mBuffers}
    { rhs.mFreeMask = ~0_u64; rhs.mBuffers = nullptr; }
    ~_BufferSubList();

    _BufferSubList& operator=(const _BufferSubList&) = delete;
    _BufferSubList& operator=(_BufferSubList&& rhs) noexcept
