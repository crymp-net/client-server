...
//     if(!func) [[unlikely]]
//     {
//         ERR("Couldn't find {} in {}", name, _wstr_to_utf8(std::data(aldriver_name)));
//         return false;
//     }
//     return true;
// }
...
//     if(!gOpenalHandle)
//     {
//         ERR("Couldn't load {}: {}", _wstr_to_utf8(std::data(aldriver_name)), GetLastError());
//         return false;
//     }
...
//     if(!ok)
//     {
//         WARN("Unloading {}", _wstr_to_utf8(std::data(aldriver_name)));
//         if(gOpenalHandle)
...
auto _wstr_to_utf8(std::wstring_view wstr) -> std::string
{
    auto ret = std::string{};
...
auto _utf8_to_wstr(std::string_view str) -> std::wstring
{
...
//             if(!f) ERR("Failed to open log file {}", _wstr_to_utf8(wstr));
//             else gLogFile = f;
//         }
